...
- По разбору кода (вопрос про `asyncio.gather`): **`gather` для DB-вызовов в наших
  сервисах неприменим** — один `AsyncSession` на запрос использует одно
  соединение, параллельный `execute()` через `gather` ведёт к «connection is
  already executing a query» и ломает атомарность аудита (аудит должен писаться
  только при успехе бизнес-мутации в той же транзакции). Верный паттерн —
  балк-запросы и батч-flush, а не `gather`.
- **A. N+1 в публичном каталоге** (`routers/events.py`): `list_events` для каждого
  события страницы делал отдельный `free_tickets(event.id)` → 1+N запросов.
  Добавлен `EventRepository.free_tickets_bulk(event_ids)` — один `GROUP BY`
  запрос; `EventService.count_free_tickets_bulk` маппит результат, уважая
  `show_free_tickets` (отдаёт `None`, если выключено). Список теперь делает
  2 запроса на выборку + 1 балк, независимо от числа событий.
- **B. Батч-создание тарифов** (`EventAdminService.create`/`clone`): цикл
  `await ticket_types.create(...)` (по flush на каждый) заменён на построение
  объектов `TicketType` + `session.add_all` + один `flush`. N раундов → 1.
- **C. Объединение чтений в `sync_price`**: `EventRepository.get_with_min_price`
  возвращает событие и минимальную цену тарифа одним `SELECT … JOIN … GROUP BY`;
  `EventAdminService._sync_price` использует его вместо двух последовательных
  чтений (`active_min_price` + `get_any`).
- **D. Лишний flush в аудите**: `AuditService.record` больше не делает
  `session.flush()` (просто `add`); строка аудита пишется итоговым `commit()`
  сервис-метода вместе с бизнес-изменением. Убран один round-trip на каждое
  аудируемое действие; последовательность и атомарность сохранены.
- Тесты: `test_free_tickets_bulk` (корректность балка + пустой вход) и
  `test_catalog_list_free_tickets_and_no_nplus1` (корректность free-счётчиков +
  подсчёт `after_cursor_execute`: запросов меньше 10 при 12 событиях, т.е. не N+1).
  Всего 47 passed ✅
